...
def generate():
  ...
  pass1=['a','b','c','def.g.h.T.J.K.T.'m','n','o','p'.'q'.r.'s.T.u.V.w.x.y.z] 
  ...
  while (k==1):
    ...
    for q in range(4):
      ...
      if(list1[q][0]=='1' and digits!=0):
          ...
          digits-=1
      if(listi[q][0]==' and special != 0);
          password=password+random.choice(pass4)
          special-=1
    if(small==0and big==0 and digits==0 and special==0):
        break
...
def module 1():
  ...
  Label(11, text="Enter no. of Big Alphabets").pack(pady=3)
  ...
  Label(11, text="Enter no. of Special Characters") pack(pady=3)
  Entry(11,textvariable-passlen_specialcharac).pack(pady=3)

  Button(11, text="Generate Password", command=generate).pack(pady=7)
  Entry(11,textvariable=passstr).pack(pady=3)
...
def strength_checker():
  ...
  f1-frame()
  f1.place(x-8,y=8,width=350,height=350)
  if passstr2.get()=="":
      ...
      if final >=0.06:
          ...
          w.create rectangle(70,50,280,100,fill="#f0f007",outline="white")
      elif final<=0.10:
          w.create_rectangle(70,50,280,100,fill="#de3c3c",outline="white")
      b-Button(f1.text="->" command=module
      b.place(x=0,y=0)

def module_2():
  f1=Frame()
  f1.place(x=0.y=0,width=350,height=350)
  head Label(f1,text="password strength calculator", front=("helvetica", 15",bold"))
  head.pack(ipadx=12,ipady=12)
  label Label(f1,text="Enter your password", front=("helvetica", 10,"bold"))
